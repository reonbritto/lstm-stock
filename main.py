...
if nav == "Stock Prediction":
    ...
    prediction_days = st.sidebar.selectbox("Forecast Horizon", options=[7, 14, 30, 60], index=2)
    time_steps = st.sidebar.selectbox("Lookback Period", options=[30, 60, 90], index=1)

    st.markdown('<h1 class="main-header">🤖 AI Stock Price Predictor</h1>', unsafe_allow_html=True)
    st.markdown("""
    <div style="text-align: center; margin-bottom: 2rem;">
        <p style="font-size: 1.2rem; color: #666;">Powered by LSTM Neural Networks</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    if st.sidebar.button("🚀 Start Analysis", type="primary", use_container_width=True):
        ...
        try:
            ...
            if stock_data is None or stock_data.empty:
                ...
                st.error(f"❌ No data found for ticker '{ticker_input}' from {start_date} to {end_date}.")
                st.stop()
            ...
            model, scaler, X_test, y_test, df_clean, feature_columns, history = train_lstm_model(stock_data, time_steps=time_steps)
            
            status_text.text("🔮 Generating predictions...")
            progress_bar.progress(80)
            future_predictions, future_dates = predict_future_prices(model, scaler, stock_data, feature_columns, days=prediction_days, time_steps=time_steps)
            ...
            if len(future_dates) != len(set(future_dates)) or len(future_dates) != len(future_predictions):
                future_dates = pd.date_range(start=stock_data.index[-1] + timedelta(days=1), periods=len(future_predictions), freq="B")
            ...
            with col1:
                ...
                fig.add_trace(go.Scatter(x=stock_data.index, y=stock_data['Close'], mode='lines', name='Historical Price', line=dict(color='#1f77b4', width=2)), row=1, col=1)
                
                if '20_MA' in stock_data.columns:
                    fig.add_trace(go.Scatter(x=stock_data.index, y=stock_data['20_MA'], mode='lines', name='20-Day MA', line=dict(color='orange', dash='dot'), opacity=0.7), row=1, col=1)
                
                if len(future_dates) == len(future_predictions):
                    fig.add_trace(go.Scatter(x=future_dates, y=future_predictions, mode='lines+markers', name='Predicted Price', line=dict(color='red', dash='dash', width=3), marker=dict(size=6)), row=1, col=1)
                    ...
                    fig.add_trace(go.Scatter(x=list(future_dates) + list(future_dates)[::-1], y=np.concatenate([upper_bound, lower_bound[::-1]]), fill='toself', fillcolor='rgba(255,0,0,0.1)', line=dict(color='rgba(255,255,255,0)'), name='Confidence Interval', showlegend=True), row=1, col=1)
                
                fig.add_trace(go.Bar(x=stock_data.index, y=stock_data['Volume'], name='Volume', marker_color='lightblue', opacity=0.6), row=2, col=1)
                fig.update_layout(height=700, title=f"{ticker_input} Stock Analysis & Prediction", xaxis_title="Date", yaxis_title="Price (USD)", yaxis2_title="Volume", hovermode='x unified', legend=dict(x=0, y=1), template="plotly_white")
                fig.update_xaxes(rangeslider_visible=True, row=1, col=1)
                st.plotly_chart(fig, use_container_width=True)
            
            with col2:
                st.subheader("📈 Model Performance")
                st.table({"MAE ($)": [f"{float(mae):.2f}"], "RMSE ($)": [f"{float(rmse):.2f}"], "MAPE (%)": [f"{float(mape):.2f}"], "R²": [f"{float(r2):.3f}"]})
                ...
                st.metric(f"Predicted Price ({prediction_days}d)", f"${predicted_price:.2f}", delta=f"{price_change_pct:+.1f}%")
    ...
    with col2:
        lookup_ticker = st.text_input("🔍 Enter Stock Ticker Symbol", value="AAPL", key="lookup_ticker", placeholder="e.g., AAPL, GOOGL, TSLA").upper().strip()
            ...
            try:
                ...
                with col1:
                    ...
                    if current_price and previous_close:
                        ...
                        st.metric("Current Price", f"${current_price:.2f}", delta=f"{price_change_pct:+.2f}%")
                    else:
                ...
                with st.expander("📰 Recent News"):
                    try:
                        feed_url = f"https://feeds.finance.yahoo.com/rss/2.0/headline?s={lookup_ticker}&region=US&lang=en-US"
                        feed = feedparser.parse(feed_url)
                        entries = feed.entries or []
                        
                        if entries:
                            st.subheader(f"Latest {len(entries)} Articles")
                            ...
                            st.info(f"No recent news found for {lookup_ticker}.")
                    except Exception as e:
                ...
                with st.expander("🎯 Analyst Recommendations"):
                    try:
                        ...
                        if recommendations is not None and not recommendations.empty:
                            ...
                            recommendations = recommendations[recommendations.index.notna()]
                            
                            if not recommendations.empty:
                                recommendations = recommendations.sort_index(ascending=False)
                                
                                # Recent recommendations (last 12 months)
                                recent_cutoff = datetime.now() - timedelta(days=365)
                                recent = recommendations[recommendations.index > recent_cutoff]
                                ...
                                if not recent.empty and 'To Grade' in recent.columns:
                                    ...
                                    if not grade_counts.empty:
                                        # Create bar chart
                                        chart_data = grade_counts.reset_index()
                                        ...
                                        st.info("No recent analyst ratings available.")
                                else:
                                    st.info("No recent analyst ratings data found.")
                                ...
                                if display_cols:
                                    ...
                                    if 'Action' in table.columns:
                                        styled_table = table.style.map(highlight_action, subset=['Action'])
                                ...
                                st.info("No valid recommendation data available.")
                        else:
                            st.info("Analyst recommendations unavailable for this stock.")
                    
                    except Exception as e:
                        st.error(f"Could not fetch analyst recommendations: {str(e)}")
    ...
    cols = st.columns(4)
    for i, stock_symbol in enumerate(popular_stocks):
        with cols[i % 4]:
            if st.button(stock_symbol, key=f"popular_{stock_symbol}"):
                st.session_state.lookup_ticker = stock_symbol
                st.rerun()